...
const useConfigureUsers = (postMessage: (message: Message) => void, postTimedMessage: (message: Message, duration: number) => void, postError: (error: SyncError) => void) => {
    ...
    const configureUsers = async (bundles: UserDataBundle[], companyERLs: ERL[], originalExtensions: Extension[], targetExtensions: Extension[]) => {
        ...
        for (const bundle of bundles) {
            await configureUser(bundle, companyERLs, originalExtensions, targetExtensions)
        }
    }
