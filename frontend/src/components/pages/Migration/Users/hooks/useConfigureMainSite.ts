...
const useConfigureMainSite = (postMessage: (message: Message) => void, postTimedMessage: (message: Message, duration: number) => void, postError: (error: SyncError) => void) => {
    const [progressValue, setProgressValue] = useState(0)
    const [maxProgress, setMaxProgress] = useState(2)
    const baseScheduleURL = 'https://platform.ringcentral.com/restapi/v1.0/account/~/business-hours'
    const baseCallHandlingURL = 'https://platform.ringcentral.com/restapi/v1.0/account/~/answering-rule/ruleId'
    const baseCustomRuleURL = 'https://platform.ringcentral.com/restapi/v1.0/account/~/answering-rule'
    const baseWaitingPeriod = 250
    
    const configureMainSite = async (bundle: SiteDataBundle, originalExtensions: Extension[], targetExtensions: Extension[]) => {
        ...
        setMaxProgress(bundle.extendedData!.customRules!.length)
    ...
    const setSchedule = async (bundle: SiteDataBundle, token: string) => {
        try {
            ...
            console.log(e)
            postMessage(new Message(`Failed to set schedule for main site. ${e.error ?? ''}`, 'error'))
    ...
    const setBusinessHoursCallHandling = async (bundle: SiteDataBundle, originalExtensions: Extension[], targetExtensions: Extension[], token: string) => {
        ...
        try {
            ...
            if (!transferID) {
                postMessage(new Message(`Business hours call handling could not be set for main site`, 'warning'))
            ...
            console.log(e)
            postMessage(new Message(`Failed to set business hours call handling for main site. ${e.error ?? ''}`, 'error'))
    ...
    const setAfterHoursCallHandling = async (bundle: SiteDataBundle, originalExtensions: Extension[], targetExtensions: Extension[], token: string) => {
        ...
        try {
            ...
            if (!transferID) {
                postMessage(new Message(`After hours call handling could not be set for site main site`, 'warning'))
            ...
            console.log(`Failed to set call handling`)
            console.log(e)
            postMessage(new Message(`Failed to set after hours call handling for main site. ${e.error ?? ''}`, 'error'))
    ...
    const getTransferExtension = (callHandling: CallHandling, originalExtensions: Extension[], targetExtensions: Extension[]) => {
        if (!callHandling.extension) return

        const originalExtension = originalExtensions.find((ext) => `${ext.data.id}` === `${callHandling.extension?.id}`)
    ...
    const addCustomRule = async (bundle: SiteDataBundle, customRule: CustomRule, token: string) => {
        try {
            ...
            console.log(e)
            postMessage(new Message(`Failed to add custom rule ${customRule.name} on main site ${e.error ?? ''}`, 'error'))
        ...
        try {
            ...
            if (customRule.calledNumbers && customRule.calledNumbers.length !== 0) {
                ...
                for (const number of customRule.calledNumbers) {
                    ...
                    if (!tempNumber) {
                        postMessage(new Message(`Failed to find temp number for called number ${number.phoneNumber} on custom rule ${customRule.name} on site main site`, 'warning'))
            ...
            if (customRule.callHandlingAction === 'TakeMessagesOnly') {
                ...
                if (!originalExtension) {
                    postMessage(new Message(`Failed to adjust voicemail recipient for custom rule ${customRule.name} on site main site`, 'error'))
                ...
                const newExtension = targetExtensions.find((ext) => ext.data.name === originalExtension.data.name && ext.prettyType() === originalExtension.prettyType())
                if (!newExtension) {
                    postMessage(new Message(`Failed to adjust voicemail recipient for custom rule ${customRule.name} on main site`, 'error'))
                    postError(new SyncError(bundle.extension.name, bundle.extension.extensionNumber, ['Failed to adjust custom rule', customRule.name]))
                    return customRule
                }
                ...
                const originalExtension = originalExtensions.find((ext) => `${ext.data.id}` === `${customRule.extension?.id}`)
                if (!originalExtension) {
                    postMessage(new Message(`Failed to adjust transfer extension for custom rule ${customRule.name} on main site`, 'error'))
                    postError(new SyncError(bundle.extension.name, bundle.extension.extensionNumber, ['Failed to adjust custom rule', customRule.name]))
                    return customRule
                }
                ...
                customRule.extension!.id = `${newExtension.data.id}`
                delete customRule.forwarding
                delete customRule.greetings
                delete customRule.unconditionalForwarding
                delete customRule.uri
            ...
            postMessage(new Message(`Failed to adjust custom rule ${customRule.name} on main site`, 'error'))
        }
    }

    return {configureMainSite, progressValue, maxProgress}
}

export default useConfigureMainSite